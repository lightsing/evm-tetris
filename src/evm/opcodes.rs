...
/// Some opcodes are disabled in tetris generation.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, PartialOrd, Ord, EnumIter)]
pub enum OpcodeId {
    // /// `STOP`
    // STOP,
    /// `ADD`
...
// code frequency from mainnet contracts
const OPCODE_ID_FREQ: [usize; 256] = [
    76384, 3160707, 210941, 1313432, 201185, 2180, 4086, 1616, 9491, 3223, 128480, 5102, 1113, 967,
    995, 1047, 272540, 312373, 160493, 5647, 684997, 1416131, 1632270, 113978, 2640, 229861, 5382,
    1089127, 43280, 1725, 1040, 1781, 383526, 1072, 14113, 1440, 981, 911, 926, 1351, 1600, 1243,
    1046, 948, 1203, 1220, 2380, 1277, 80142, 2171, 11794, 165173, 294679, 228044, 260353, 24806,
    2991, 22445, 8023, 33940, 1366, 234547, 83620, 931, 992, 3138, 23012, 14990, 1464, 6822, 1875,
    24577, 1020, 1720, 866, 927, 1354, 1652, 1529, 1500, 4466336, 1406386, 2631276, 5719, 949030,
    307518, 3828941, 2123390, 1011, 1212, 76475, 5313314, 931, 938, 920, 2125, 11736274, 7632422,
    292251, 708567, 38801, 18171, 9097, 58701, 12414, 8025, 3624, 6716, 5541, 7347, 5598, 9220,
    4448, 3184, 6910, 499128, 9837, 6275, 1955, 2327, 2680, 962, 1938, 5275, 2601, 915, 2437,
    494947, 3283907, 3064892, 2173159, 1143714, 729040, 486858, 300467, 211722, 114858, 72295,
    67954, 42186, 26993, 33120, 11781, 8633, 4157493, 2042150, 727052, 276152, 132958, 82388,
    55239, 25490, 26945, 10914, 8990, 8387, 10528, 2164, 3171, 1806, 948, 16319, 18702, 46668,
    6715, 955, 1003, 851, 1543, 949, 1525, 1110, 919, 1083, 875, 925, 910, 983, 917, 1397, 895,
    843, 909, 1526, 1142, 947, 854, 1026, 912, 924, 998, 1032, 1530, 863, 892, 832, 850, 918, 838,
    963, 1106, 1015, 831, 806, 3259, 1630, 918, 1047, 909, 901, 1162, 900, 1015, 894, 794, 824,
    951, 913, 855, 914, 859, 1055, 878, 830, 1156, 987, 1108, 885, 825, 862, 1005, 872, 937, 1494,
    1018, 804, 839, 922, 897, 1115, 1891, 48848, 1200, 101249, 4103, 1531, 1037, 1486, 856, 996,
    33503, 1103, 1423, 864130, 32426, 1040,
];

static OPCODES: Lazy<Vec<OpcodeId>> = Lazy::new(|| OpcodeId::iter().collect());
static OPCODE_DIST: Lazy<WeightedIndex<usize>> = Lazy::new(|| {
    WeightedIndex::new(OpcodeId::iter().map(|opcode| OPCODE_ID_FREQ[opcode.as_usize()])).unwrap()
});
...
impl OpcodeId {
    ...
    pub fn random(mut rng: impl RngCore) -> Self {
        OPCODES[OPCODE_DIST.sample(&mut rng)]
    }
}
...
impl From<u8> for OpcodeId {
    fn from(value: u8) -> Self {
        match value {
            ...
            _ => panic!("Invalid or Disabled opcode: {}", value),
        }
    }
}
